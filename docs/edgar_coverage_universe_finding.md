...
> **KNOWN GAP:** amendment_log has 3 entries; the 3rd entry's own text references itself as the "Fourth amendment." A real 4th amendment occurred (confirmed via git commit history) but its dedicated log entry was never persisted or was lost. Unrecoverable from store.duckdb (gitignored). Documented as a known limitation of the audit trail as of 2026-07-14, not fabricated. Going forward: `verify_prereg()` and the amendment count should be reconciled against git log for pre-registration-related commits whenever a new amendment is drafted, until this is either explained or formally noted as accepted loss.
